...
## 9. Test and smoke surface — pass 8

- Evidence: `package.json:6-13` — `test: vitest run` and `typecheck: tsc --noEmit` are available; there is still no `e2e` script.
- Evidence: `vitest.config.ts` — Node test environment, `@` alias, `.test.tsx` include, and OXC automatic JSX transform are configured for hermetic route-handler/unit/server-render component tests.
...
- Evidence: `src/components/report/ReportHeader.test.tsx` — weekly/daily/initial header labels, period copy, report link, and hidden-link behavior are covered through server-render tests.
- Evidence: `src/components/report/highlight/SummaryAccordion.test.tsx` and `src/components/report/strategy/StrategyCard.test.tsx` — interactive collapsed state vs PDF-mode expanded summary/strategy content is covered.
- Evidence: `src/components/report/risk-content/RiskTable.test.tsx` — risk empty-state and PDF row-limit summary rendering are covered with mutation/modal dependencies mocked.
...
3. Report UI regression: broader report section/page integration rendering, channel/risk drawer open-only-when-data rules, and additional PDF-mode row/content limits; header/summary/strategy/risk-table smoke coverage now exists.
